...
the input. Should ungets know about buf and bufp, or should it
just use ungetch?

ungetch() is currently set up to dismiss characters that would exceed
the amount that can be stored. Since there is already error checking in
ungetch(), ungetch() should be used to add to the buffer. It is not
necessary that ungets knows about buf and bufp.
*/
...
/**
pushes this string back onto the input. If pushing the character
would exceed the input's limit, then that character is ignored.
*/
void ungets(char s[]) {
    int i;
